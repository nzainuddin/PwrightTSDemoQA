...
test('Verify successfully register and log into account', async ({ pages, controllerAPI }) => {
  await pages.loginPage.navigateToRegisterPage();
  await pages.registerPage.register('Matthew', 'Simpsons', 'mattson', 'Mattson05$');
  await pages.loginPage.login('mattson', 'Mattson05$');
  await expect(pages.profilePage.usernameLbl).toHaveText('Matthew');
